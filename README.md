...
测试环境：3 个异构 Mock Backend（A100 ×2 token 10-30ms, A10 ×1 token 80-150ms），c=10，hey 30s 压测。

| 路由策略 | 平均延迟 | P99 延迟 | QPS | 总请求数 |
|---------|:------:|:------:|:---:|:------:|
| 加权路由 | 54ms | 266ms | 184.6 | 5561 |
| Round-Robin | 104ms | 285ms | 96.0 | 2901 |
| 差异 | **-47.9%** | -6.8% | **+92.2%** | — |

关键结论：

- **加权路由 QPS 几乎翻倍**：加权打分自动将 ~75% 流量导向高性能 A100，避免了 Round-Robin 将 1/3 流量浪费在慢速 A10 上
- **P99 差距较小**：因为 A10 即使较少命中也是慢的，两条曲线的尾部由 A10 决定；但在真实场景中加权路由 P99 会低更多（可以给 A10 配更低的 max_requests）
- **平均延迟降低 48%**：加权路由让大多数请求落在 A100（10-30ms/token），Round-Robin 均匀分配导致更多请求撞上慢速 A10（80-150ms/token）
